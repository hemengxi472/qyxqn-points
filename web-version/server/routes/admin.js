...
router.post('/reviews/:id/action', (req, res) => {
  ...
  const tx = db.transaction(() => {
    ...
    if (action === 'approve') {
      ...
      const summary = db.prepare('SELECT * FROM points_summary WHERE user_id = ?').get(sub.user_id);
...
router.get('/employees', (req, res) => {
  ...
  const items = users.map(u => {
    const summary = db.prepare('SELECT total_points FROM points_summary WHERE user_id = ?').get(u.id);
    return {
      id: u.id,
...
router.get('/stats', (_req, res) => {
  const empCount = db.prepare('SELECT COUNT(*) as cnt FROM users WHERE status = ?').get('active').cnt;
  const subTotal = db.prepare('SELECT COUNT(*) as cnt FROM submissions').get().cnt;
  const pendingCount = db.prepare("SELECT COUNT(*) as cnt FROM submissions WHERE status = 'pending'").get().cnt;

  const modules = db.prepare('SELECT id, name FROM modules WHERE is_active = 1 ORDER BY sort_order').all();
  const allSummaries = db.prepare('SELECT * FROM points_summary').all();
  ...
  const topEmployees = allSummaries
    .sort((a, b) => (b.total_points || 0) - (a.total_points || 0))
    .slice(0, 10)
    .map(s => {
      const emp = db.prepare('SELECT name, department FROM users WHERE employee_id = ?').get(s.employee_id);
      return {
  ...
  const fraudCount = db.prepare('SELECT COUNT(*) as cnt FROM fraud_records').get().cnt;
  ...
  const submittedGroups = db.prepare(`
...
router.post('/employees/:id/promote', superAdminMiddleware, (req, res) => {
...
router.post('/employees/:id/disable', superAdminMiddleware, (req, res) => {
  const user = db.prepare('SELECT * FROM users WHERE id = ?').get(req.params.id);
  if (!user) return res.status(404).json({ message: '员工不存在' });
...
router.post('/groups/generate', (req, res) => {
  ...
  const existing = db.prepare('SELECT COUNT(*) as cnt FROM groups WHERE month_year = ?').get(monthYear);
  if (existing.cnt > 0) {
    return res.status(400).json({ message: `${monthYear} 团队已生成，如需重新生成请先删除` });
  }

  const employees = db.prepare("SELECT * FROM users WHERE status = 'active' AND role = 'employee' ORDER BY RANDOM()").all();
  ...
  while (idx < shuffled.length) {
    ...
    let size = 4; // preferred size
    if (remaining < 3) {
      // Distribute remaining 1-2 people to existing groups
...
router.get('/groups/:id', (req, res) => {
  ...
  const submission = group.submission_id
    ? db.prepare('SELECT * FROM submissions WHERE id = ?').get(group.submission_id)
    : null;
...
router.post('/groups/:id/review', (req, res) => {
  ...
  const submission = db.prepare('SELECT * FROM submissions WHERE id = ?').get(group.submission_id);
  if (!submission) return res.status(400).json({ message: '提交记录不存在' });

  const reviewer = db.prepare('SELECT * FROM users WHERE id = ?').get(req.user.id);
  const allMembers = db.prepare('SELECT * FROM group_members WHERE group_id = ?').all(group.id);
  const now = new Date().toISOString().replace('T', ' ').substring(0, 19);
  ...
  const tx = db.transaction(() => {
    if (action === 'approved') {
      ...
      for (const m of allMembers) {
        ...
        const summary = db.prepare('SELECT * FROM points_summary WHERE user_id = ?').get(m.user_id);
        if (!summary) {
          db.prepare(`
            INSERT INTO points_summary (user_id, employee_id, total_points, module_points, updated_at)
            VALUES (?, ?, ?, ?, ?)
        ...
        const mpRow = db.prepare('SELECT * FROM monthly_points WHERE user_id = ? AND month_year = ?')
          .get(m.user_id, group.month_year);
        if (mpRow) {
          if (!mpRow.is_fraud_reset) {
            db.prepare('UPDATE monthly_points SET points = points + ?, updated_at = ? WHERE id = ?')
              .run(points, now, mpRow.id);
          }
        } else {
          db.prepare('INSERT INTO monthly_points (user_id, employee_id, month_year, points) VALUES (?, ?, ?, ?)')
...
router.post('/tasks', (req, res) => {
  ...
  const existing = db.prepare('SELECT * FROM monthly_tasks WHERE month_year = ?').get(monthYear);
  const now = new Date().toISOString().replace('T', ' ').substring(0, 19);

  if (existing) {
    db.prepare('UPDATE monthly_tasks SET task_description = ?, created_by = ?, created_at = ? WHERE id = ?')
      .run(taskDescription.trim(), req.user.id, now, existing.id);
  } else {
    db.prepare('INSERT INTO monthly_tasks (month_year, task_description, created_by, created_at) VALUES (?, ?, ?, ?)')
      .run(monthYear, taskDescription.trim(), req.user.id, now);
  }
...
router.get('/tasks', (_req, res) => {
  const tasks = db.prepare('SELECT * FROM monthly_tasks ORDER BY month_year DESC').all();
  res.json({
    tasks: tasks.map(t => ({
      id: t.id,
...
router.get('/tasks/:monthYear', (req, res) => {
  const task = db.prepare('SELECT * FROM monthly_tasks WHERE month_year = ?').get(req.params.monthYear);
  res.json({
    task: task ? {
      id: task.id,
...
router.post('/fraud', (req, res) => {
  ...
  const tx = db.transaction(() => {
    ...
    // Zero monthly points
    const mpRow = db.prepare('SELECT * FROM monthly_points WHERE user_id = ? AND month_year = ?').get(userId, monthYear);
    if (mpRow) {
      db.prepare('UPDATE monthly_points SET points = 0, is_fraud_reset = 1, updated_at = ? WHERE id = ?')
        .run(now, mpRow.id);
    } else {
      db.prepare('INSERT INTO monthly_points (user_id, employee_id, month_year, points, is_fraud_reset) VALUES (?, ?, ?, 0, 1)')
        .run(userId, user.employee_id, monthYear);
    }

    // Deduct from points_summary
    if (totalMonthPoints > 0) {
      const summary = db.prepare('SELECT * FROM points_summary WHERE user_id = ?').get(userId);
      if (summary) {
        const mp = JSON.parse(summary.module_points || '{}');
        // Deduct proportionally from each module
        const modPoints = {};
        monthLogs.each = undefined;
        ...
        db.prepare('UPDATE points_summary SET total_points = ?, module_points = ?, updated_at = ? WHERE user_id = ?')
...
router.delete('/fraud/:id', superAdminMiddleware, (req, res) => {
  const fraud = db.prepare('SELECT * FROM fraud_records WHERE id = ?').get(req.params.id);
  ...
  const tx = db.transaction(() => {
    ...
    db.prepare('UPDATE monthly_points SET points = ?, is_fraud_reset = 0, updated_at = ? WHERE user_id = ? AND month_year = ?')
      .run(recalculated, now, fraud.user_id, fraud.month_year);

    // Restore points_summary
    const summary = db.prepare('SELECT * FROM points_summary WHERE user_id = ?').get(fraud.user_id);
    if (summary && recalculated > 0) {
      ...
      db.prepare('UPDATE points_summary SET total_points = total_points + ?, module_points = ?, updated_at = ? WHERE user_id = ?')
        .run(recalculated, JSON.stringify(mp), now, fraud.user_id);
    }

    db.prepare('DELETE FROM fraud_records WHERE id = ?').run(req.params.id);
  });
  tx();
...
router.get('/modules/:moduleId/subcategories', (req, res) => {
...
router.put('/subcategories/:id/toggle', (req, res) => {
  const sub = db.prepare('SELECT * FROM subcategories WHERE id = ?').get(req.params.id);
  if (!sub) return res.status(404).json({ message: '子项不存在' });

  const newState = sub.is_active ? 0 : 1;
  db.prepare('UPDATE subcategories SET is_active = ? WHERE id = ?').run(newState, req.params.id);
  res.json({ success: true, isActive: !!newState });
});
